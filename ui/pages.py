...
class HomePage(tk.Frame):
    ...
    def _show_strategy_modal(self, on_selected):
        ...
        options = [
            ...
            ("DP & Backtracking", "dp_backtracking"),
        ]
...
class GamePage(tk.Frame):
    ...
    def _handle_no_cpu_move(self, strategy_source):
        if not self.game_state.game_over:
            ...
            if self.game_state.consecutive_passes >= 2:
                ...
                self.game_state.winner = "Stalemate"
                self.game_state.message = "Game Over! Both AIs continuously passed."
                self.check_game_over()
                return
            ...
            # In AI vs AI mode, trigger the next CPU's move even if this one passed
            if self.game_state.game_mode == "ai_vs_ai" and "CPU" in self.game_state.turn:
                self.after(500, self.cpu_move)
    ...
    def _get_strategy_display_name(self):
        ...
        if strategy == "dp_backtracking":
            return "DP & Backtracking"
...
class StatsPage(tk.Frame):
    def __init__(self, master, on_back):
        ...
        tk.Label(self, text="Statistics", font=FONT_TITLE, bg=BG_COLOR, fg=TEXT_COLOR).pack(pady=20)

        # Dashboard (separate component for clarity)
        try:
            from ui.statistics_panel import StatisticsDashboard
            dashboard = StatisticsDashboard(self)
            dashboard.pack(expand=True, fill=tk.BOTH, padx=20, pady=10)
        except Exception:
            # Fallback: show a simple card with basic values
        ...
        HoverButton(self, text="Back", command=on_back).pack(pady=12)
