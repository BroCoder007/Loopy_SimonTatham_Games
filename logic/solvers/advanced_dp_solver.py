...
class AdvancedDPSolver(AbstractSolver):
    ...
    def decide_move(self) -> Tuple[List[Tuple[Tuple[int, int], int]], Optional[Tuple[int, int]]]:
        ...
        # Priority 1: Remove wrong edges (correction)
        for edge in all_edges:
            if edge in current_edges and edge not in self._final_solution_edges:
                valid, reason = is_valid_move(self.game_state.graph, edge[0], edge[1], self.game_state.clues)
                if valid:
                    self.last_explanation = "Advanced DP determined this edge violates the global loop and must be removed."
                    move = edge
                    ...
                    from logic.execution_trace import log_advanced_dp_move
                    log_advanced_dp_move(move=move, explanation=self.last_explanation, recursion_depth=0, region_id="Global")
                    return [(move, 100)], move

        # Priority 2: Adding edges
        for edge in all_edges:
            if edge in self._final_solution_edges and edge not in current_edges:
                valid, reason = is_valid_move(self.game_state.graph, edge[0], edge[1], self.game_state.clues)
                if valid:
                    self.last_explanation = "Advanced DP determined this edge is globally forced."
                    move = edge
                    from logic.solvers.solver_interface import MoveExplanation
                    self._last_move_metadata = MoveExplanation(
                        mode="Advanced DP",
